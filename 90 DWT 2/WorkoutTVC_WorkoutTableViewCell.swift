...
class WorkoutTVC_WorkoutTableViewCell: UITableViewCell, UITextFieldDelegate {
    ...
    @IBAction func saveCurrentRep1(_ sender: UITextField) {
        ...
        if (sender.text?.count > 0 && sender.text != "0.0" && sender.text != originalCurrentRep1_Text) {
            ...
            CDOperation.saveRepsWithPredicate(session, workout: selectedWorkout, month: month, week: workoutWeek, exercise: nonUpperCaseExerciseName, index: workoutIndex as NSNumber, reps: sender.text!, round: "Round 1")
        }
        else {
    ...
    @IBAction func saveCurrentWeight1(_ sender: UITextField) {
        ...
        if (sender.text?.count > 0 && sender.text != "0.0" && sender.text != originalCurrentWeight1_Text) {
            ...
            CDOperation.saveWeightWithPredicate(session, workout: selectedWorkout, month: month, week: workoutWeek, exercise: nonUpperCaseExerciseName, index: workoutIndex as NSNumber, weight: sender.text!, round: "Round 1")
        }
        else {
    ...
    @IBAction func saveCurrentRep2(_ sender: UITextField) {
        ...
        if (sender.text?.count > 0 && sender.text != "0.0" && sender.text != originalCurrentRep2_Text) {
            ...
            CDOperation.saveRepsWithPredicate(session, workout: selectedWorkout, month: month, week: workoutWeek, exercise: nonUpperCaseExerciseName, index: workoutIndex as NSNumber, reps: sender.text!, round: "Round 2")
        }
        else {
    ...
    @IBAction func saveCurrentWeight2(_ sender: UITextField) {
        ...
        if (sender.text?.count > 0 && sender.text != "0.0" && sender.text != originalCurrentWeight2_Text) {
            ...
            CDOperation.saveWeightWithPredicate(session, workout: selectedWorkout, month: month, week: workoutWeek, exercise: nonUpperCaseExerciseName, index: workoutIndex as NSNumber, weight: sender.text!, round: "Round 2")
        }
        else {
    ...
    @IBAction func saveCurrentRep3(_ sender: UITextField) {
        ...
        // Only update the fields that have been changed.
        if (sender.text?.count > 0 && sender.text != "0.0" && sender.text != originalCurrentRep3_Text) {
    ...
    @IBAction func saveCurrentWeight3(_ sender: UITextField) {
        ...
        // Only update the fields that have been changed.
        if (sender.text?.count > 0 && sender.text != "0.0" && sender.text != originalCurrentWeight3_Text) {
    ...
    @IBAction func saveCurrentNotes(_ sender: UITextField) {
        ...
        if (sender.text?.count > 0 && sender.text != "CURRENT NOTES" && sender.text != originalCurrentNotes_Text) {
            ...
            CDOperation.saveNoteWithPredicate(session, workout: selectedWorkout, month: month, week: workoutWeek, exercise: nonUpperCaseExerciseName, index: workoutIndex as NSNumber, note: sender.text!, round: "Round 1")
        }
        else {
